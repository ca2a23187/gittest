...
int removeFrequnce(int x, cv::Mat srcMat)
{
	...
	calcVisbalDft(srcMat, magMat, phMat, normVal, srcMat);

	//在幅值谱上，通过鼠标选择，需要去掉的频率
	selectPolygon1(magMat, maskMat, x);
...
void on_mouse1(int EVENT, int x, int y, int flags, void* userdata)
{
	...
	case EVENT_LBUTTONDOWN:
	{
		points1.x = x;
		points1.y = y;
		mousePoints1.push_back(points1);
		circle(hh, points1, 4, cvScalar(255, 255, 255), -1);
		imshow("mouseCallback", hh);
	}
	break;
	}
...
int selectPolygon1(cv::Mat srcMat, cv::Mat &dstMat, int x)
{
	...
	else
	{
		namedWindow("mouseCallback");
		imshow("mouseCallback", selectMat);
		setMouseCallback("mouseCallback", on_mouse1, &selectMat);
		waitKey(0);
		destroyAllWindows();
		//计算roi
		contours1.push_back(mousePoints1);
		if (contours1[0].size() < 3) {
			std::cout << "failed to read image!:" << std::endl;
			return -1;
		}
	}
	...
	if (x == 1) {
		...
		drawContours(m2, contours1, 0, Scalar(1), -1);
		m2.copyTo(dstMat);
		//imshow("m2", m2);
	}
...
int mouseROI(int x, cv::Mat srcMat)
{
	...
	selectPolygon1(srcMat, dstMat, x);
...
int ifftDemo(Mat src, int x, cv::Mat &dspMat)
{
	cv::Mat dst;
	...
	cv::Mat mask1;
	Mat proceMag;
	//imshow("处理后频谱12.jpg", mag);
	if (x == 1) {
		selectPolygon1(mag, mask, x);
		//cout << "mask" << mask;
		//imshow("处理后频谱11.jpg", mask);
		//imshow("处理后频谱1.jpg", mag);
		mag = mag.mul(mask);
		//imshow("处理后频谱1.jpg", mag);
	}
	else
	{
		selectPolygon1(mag, mask1, x);
		//cout << "mask" << mask;
		//imshow("处理后频谱11.jpg", mask1);
		mag = mag.mul(mask1);
		//imshow("处理后频谱1.jpg", mag);
	}
	//imshow("处理后频谱12.jpg", mag);
